...
const UUID = "desktopNotes@MCHilli";
...
class MyDesklet extends Desklet.Desklet {
    constructor(metadata, instanceId) {
        ...
        try {
            this.settings = new Settings.DeskletSettings(
                this, this.metadata["uuid"], instanceId);

            this.settings.bindProperty(Settings.BindingDirection.IN, "note-from-file", "noteFromFile", this.on_setting_changed);
            this.settings.bindProperty(Settings.BindingDirection.IN, "note-content", "noteContent", this.on_setting_changed);
            this.settings.bindProperty(Settings.BindingDirection.IN, "file-path", "filePath", this.on_setting_changed);
    ...
    on_file_changed() {
        ...
        if (this.signalManager.getSignals().length === 0) {
            ...
            this.signalManager.connect(fileMonitor, "changed", (filemonitor, file, other_file, event_type) => {
                if (event_type === 1){
                    this.on_file_changed();
                }
            });
